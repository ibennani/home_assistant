#!/bin/bash
# Z-Wave JS UI migration via docker exec. Loggar till /config/zwave-migrate.log
# och system_log (sök ZWAVE_MIGRATE i HA-loggar).
set -uo pipefail
...
for f in nodes.json users.json scenes.json groups.json; do
  for src in "$CORE/$f" "$CORE/store/$f"; do
    if [[ -f "$src" ]]; then
      ...
      log_ha "copied $f $(wc -c < "$src") bytes"
      copied=1
      break
    fi
  done
...
[[ -d "$CORE/.config-db" ]] && docker cp "$CORE/.config-db/." "$CID:/data/db/" && log_ha "copied config-db"
...
log_ha "store listing: $store_list"
...
log_ha "restarting UI addon"
ha addons restart "$SLUG"
sleep 35

tail_log=$(ha addons logs "$SLUG" 2>&1 | tail -15 | tr '\n' ' ')
