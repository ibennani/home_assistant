...
#
# Kör från valfri plats:  bash scripts/verify-ha-mcp.sh
#
# Läser konfiguration i denna ordning:
#   1. HA_URL + HA_TOKEN (från .env eller miljön)
#   2. Fallback: en miljövariabel vars NAMN är HA:s https-URL och vars
#      VÄRDE är en long-lived token (så här lagras Cloud Agent-hemligheten).
#   3. MCP-webhook: HA_MCP_WEBHOOK_URL, annars home-assistant-URL i
#      .cursor/mcp.json om filen finns.
set -uo pipefail
...
# Fallback: hitta en hemlighet där namnet = HA-URL och värdet = token.
if [[ -z "${HA_TOKEN:-}" ]]; then
...
# MCP-webhook-URL: explicit variabel eller från .cursor/mcp.json.
...
TOTAL=0
check() {
    # check "namn" faktisk "förväntat1 förväntat2 ..." "detalj"
...
if [[ "$PASS" -eq "$TOTAL" ]]; then
    ...
    echo
    echo "REST API-anslutningen fungerar. För Cursor Cloud Agent / Automation:"
    echo "  Lägg till MCP-servern 'home-assistant' på https://cursor.com/settings (MCP)"
    echo "  med Nabu Casa-webhook-URL:en. Se docs/cursor-cloud-mcp-steg.md."
    exit 0
fi
